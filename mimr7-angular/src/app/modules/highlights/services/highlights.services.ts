...
export class HighlightService {
  private highlightState = new BehaviorSubject<Highlight[]>([]);
  ...
  setHighlightState(highlight: Highlight[]) {
    this.highlightState.next(highlight);
  }

  addHighlight(data: Highlight): Observable<any> {
  ...
  getAll(
    ...
    const url = withQuery(
      `${environment.apiUrl}highlights?offset=${pageNumber}&limit=${pageSize}&content=${content}`,
      query
    );
  ...
  updateHighlight(id: string, data: Highlight) {
