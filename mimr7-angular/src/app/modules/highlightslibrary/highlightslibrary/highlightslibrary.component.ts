...
@Component({
  selector: 'app-highlightslibrary',
  templateUrl: './highlightslibrary.component.html',
  styleUrls: ['./highlightslibrary.component.scss']
})
export class HighlightslibraryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbService.setrouteState('Highlights Library');
    this.getAll();
  }

  
  getAll() {
    this.highlightLibraryService.getAll(1, 10, this.currentSort).subscribe((res) => {
      console.log(res);
    });
    this.data = combineLatest(this.currentSort, this.page).pipe(
      switchMap(([sortChange, page]) => {
        ...
        return this.highlightLibraryService.getAll(
          page.pageIndex + 1,
          page.pageSize,
          sortChange,
          this.title
